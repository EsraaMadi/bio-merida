...
from . import logging
from .data import BitFitDataset
...
This is a [BitFit model](https://github.com/huggingface/BitFit) that can be used for text classification. \
The model has been trained using an efficient few-shot learning technique that involves:

1. Fine-tuning a [Sentence Transformer](https://www.sbert.net) with contrastive learning.
2. Training a classification head with features from the fine-tuned Bio Transformer.
"""


class BitFitBaseModel:
...
class BitFitHead(models.Dense):
    """
    A BitFit head that supports multi-class classification for end-to-end training.
    ...
    Args:
        in_features (`int`, *optional*):
            The embedding dimension from the output of the BitFit body. If `None`, defaults to `LazyLinear`.
    ...
    def forward(
        ...
        """
        BitFitHead can accept embeddings in:
    ...
    def __repr__(self):
        return "BitFitHead({})".format(self.get_config_dict())


@dataclass
class BitFitModel(PyTorchModelHubMixin):
    """A BitFit model with integration to the Hugging Face Hub."""
    ...
    def to(self, device: Union[str, torch.device]) -> "BitFitModel":
        """Move this BitFitModel to `device`, and then return `self`. This method does not copy.

        Args:
            device (Union[str, torch.device]): The identifier of the device to move the model to.

        Returns:
            BitFitModel: Returns the original model, but now on the desired device.
        """
    ...
    def create_model_card(self, path: str, model_name: Optional[str] = "BitFit Model") -> None:
        """Creates and saves a model card for a BitFit model.

        Args:
            path (str): The path to save the model card to.
            model_name (str, *optional*): The name of the model. Defaults to `BitFit Model`.
        """
        ...
        **model_kwargs,
        ...
        if model_head_file is not None:
            ...
            if use_differentiable_head:
                ...
                model_head = BitFitHead(**{**head_params, **base_head_params})
            else:
...
def sequence_pairs_generation(sequences, labels, pairs):
...
def sequence_pairs_generation_multilabel(sequences, labels, pairs):
    # Initialize two empty lists to hold the (sentence, sentence) pairs and
    # labels to indicate if a pair is positive or negative
    for first_idx in range(len(sequences)):
        current_sentence = sequences[first_idx]
...
def sequence_pairs_generation_cos_sim(sequences, pairs, cos_sim_matrix):
    # initialize two empty lists to hold the (sentence, sentence) pairs and
    # labels to indicate if a pair is positive or negative
    ...
    for first_idx in range(len(sequences)):
        ...
        paired_sentence = sequences[second_idx]
        ...
        paired_sentence = sequences[third_idx]
        pairs.append(InputExample(texts=[current_sentence, paired_sentence], label=cos_sim))
...
class SKLearnWrapper:
    ...
    def save(self, path):
        ...
        joblib.dump(self.clf, f"{path}/bitfit_head.pkl")

    def load(self, path):
        self.st_model = SentenceTransformer(model_name_or_path=path)
        self.clf = joblib.load(f"{path}/bitfit_head.pkl")
