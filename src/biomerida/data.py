...
class BitFitDataset(TorchDataset):
    """BitFitDataset

    A dataset for training the differentiable head on sequence classification.

    Args:
        x (`List[str]`):
            A list of input data as texts that will be fed into `BitFitModel`.
        y (`Union[List[int], List[List[int]]]`):
            A list of input data's labels. Can be a nested list for multi-label classification.
        tokenizer (`PreTrainedTokenizerBase`):
            The tokenizer from `BitFitModel`'s body.
