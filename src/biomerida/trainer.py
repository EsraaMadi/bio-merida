...
if TYPE_CHECKING:
    ...
    from .modeling import BitFitModel
...
class BitFitTrainer:
    """Trainer to train a BitFit model.

    Args:
        model (`BitFitModel`, *optional*):
            The model to train. If not provided, a `model_init` must be passed.
        train_dataset (`Dataset`):
            The training dataset.
        eval_dataset (`Dataset`, *optional*):
            The evaluation dataset.
        model_init (`Callable[[], BitFitModel]`, *optional*):
            A function that instantiates the model to be used. If provided, each call to [`~BitFitTrainer.train`] will start
    ...
    def __init__(
        self,
        model: Optional["BitFitModel"] = None,
        train_dataset: Optional["Dataset"] = None,
        eval_dataset: Optional["Dataset"] = None,
        model_init: Optional[Callable[[], "BitFitModel"]] = None,
        ...
        if model is None:
            if model_init is not None:
                ...
                raise RuntimeError("`BitFitTrainer` requires either a `model` or `model_init` argument")
        else:
            if model_init is not None:
                raise RuntimeError("`BitFitTrainer` requires either a `model` or `model_init` argument, but not both")
        ...
        for key, value in params.items():
            if hasattr(self, key):
                ...
                logger.warning(
                    ...
                    "`BitFitTrainer`, and `model_init` does not take any arguments."
                )
    ...
    def freeze(self):
        """
        Freeze BitFitModel's differentiable head.
        Note: call this function only when using the differentiable head.
        """
        if not self.model.has_differentiable_head:
            raise ValueError("Please use the differentiable head in `BitFitModel` when calling this function.")
    ...
    def unfreeze(self, keep_body_frozen: bool = False):
        """
        Unfreeze BitFitModel's differentiable head.
        Note: call this function only when using the differentiable head.
    ...
    def train(
        ...
        Args:
            ...
            learning_rate (`float`, *optional*):
                Temporary change the learning rate to use for contrastive training or BitFitModel's head in logistic regression.
                If ignore, will use the value given in initialization.
            body_learning_rate (`float`, *optional*):
                Temporary change the learning rate to use for BitFitModel's body in logistic regression only.
                If ignore, will be the same as `learning_rate`.
            l2_weight (`float`, *optional*):
                Temporary change the weight of L2 regularization for BitFitModel's differentiable head in logistic regression.
        ...
        if self.train_dataset is None:
            raise ValueError("Training requires a `train_dataset` given to the `BitFitTrainer` initialization.")
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        To use this method, you need to have provided a `model_init` when initializing your [`BitFitTrainer`]: we need to
    ...
    def push_to_hub(
        ...
        commit_message: Optional[str] = "Add BitFit model",
